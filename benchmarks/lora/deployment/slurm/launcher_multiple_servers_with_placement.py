...
EXP_SLURM_EXECUTABLE = 'benchmarks/lora/deployment/slurm/slurm_multiple_servers.sh'

# path to benchmark executable
EXP_BENCHMARK_EXECUTABLE = 'benchmarks/lora/benchmark_serving_by_time_multiple_servers_with_placement.py'
...
def schedule_job(
    ...
    assert '--num-servers' in arguments
    pattern = r"--num-servers='(\d+)'"
    match = re.search(pattern, arguments)
    num_gpus: int = int(match.group(1))
    num_cpus: int = num_gpus * 20
    assert 0 < num_gpus <= 4
    env['GPUS'] = str(num_gpus)
    env['CPUS'] = str(num_cpus)
        ...
        for benchmark_index, benchmark_args_combination in enumerate(benchmark_args_combinations):
            ...
            if runs is not None:
                for run in range(runs):
                    ...
                    arguments = f"--port='{random_port}' --result-dir='{os.path.join(results_path, run_combination_game)}' {benchmark_args_combination} --server-args='{server_args_combination}'"
                    schedule_job(
                        user,
                        queue,
                ...
                arguments = f"--port='{random_port}' --result-dir='{os.path.join(results_path, combination_name)}' {benchmark_args_combination} --server-args='{server_args_combination}'"
                schedule_job(
                    user,
                    queue,
                    combination_name,
