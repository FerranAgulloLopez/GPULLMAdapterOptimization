...
TESTED_ADAPTERS: List[int] = [8, 16, 32, 64, 96, 128, 160, 192, 256, 320, 384, 448, 512, 576]
TESTED_SLOTS: List[int] = [8, 16, 32, 64, 96, 128, 160, 192, 256, 320, 384, 448, 512, 576]
...
class PlacementAlgorithmProposalStarvation4(PlacementAlgorithmInterface):
    ...
    def define_placement(
        ...
        remaining_servers: List[Tuple[float, str]] = [(0, server) for server in servers]
        heapq.heapify(remaining_servers)
        ...
        while grouped_sorted_adapters:
            ...
            while group_adapters:
                ...
                server_aggregated_rate, server = heapq.heappop(remaining_servers)
                ...
                server_aggregated_rate += adapter_rate
                ...
                if not server_starved:
                    heapq.heappush(remaining_servers, (server_aggregated_rate, server))
    ...
    def __try_server_allocation(
        ...
        if input_ml_data_more_slots is not None:
            adapter_slots = input_ml_data_more_slots['adapter_slots']
            max_size = input_ml_data_more_slots['max_size']
            assert max_size in GPU_MEMORY_AVAILABILITY[model]
            memory_error = False if adapter_slots <= GPU_MEMORY_AVAILABILITY[model][max_size] else True
            if memory_error:
                input_ml_data_more_slots = None

        if input_ml_data is None and input_ml_data_more_slots is None:
            starvation = True
        ...
        if not starvation:
            if input_ml_data is not None:
                output_ml_data: Dict[str, Any] = predict_digital_twin(
                    regression=True,
                    model_path=ml_model_path_throughput,
            ...
            if total_throughput >= total_throughput_more_slots:
                ...
                input_ml_data = input_ml_data_more_slots
