...
ML_MODEL_PATH: Dict[str, str] = {
    'llama-3.1-8b-instruct': '/gpfs/scratch/bsc98/bsc098069/experiment_data/llm_benchmarking/models/trained_ml/llama-31-8b-instruct/reg/rf'
}
TESTED_ADAPTERS: List[int] = [8, 16, 32, 64, 96, 128, 160, 192, 256, 320, 384]
TESTED_SLOTS: List[int] = [8, 16, 32, 64, 96, 128, 160, 192, 256, 320, 384]


class PlacementAlgorithmProposalThroughput(PlacementAlgorithmInterface):
                ...
                if testing_point:
                    ...
                    if allocated:
                        ...
                        for index, adapter in enumerate(pending_adapters):
                            ...
                            leftover_grouped_adapters[pending_ranks[index]].append((pending_rates[index], adapter))
                        ranks: List[int] = list(leftover_grouped_adapters.keys())
                        ranks = sorted(ranks, reverse=True)  # order in reverse to properly include ranks in original order
                        for rank in ranks:
    ...
    def __try_server_allocation(
        ...
        assert server_init_workload['remaining_adapter_slots_to_test'][0] <= server_init_workload['remaining_served_adapters_to_test'][0]
        ...
        if server_init_workload['served_adapters'] + len(server_init_workload['pending_adapters']) < server_init_workload['remaining_served_adapters_to_test'][0]:
        ...
        if server_init_workload['adapter_slots'] > 0:
            ...
            input_ml_data = None
        if server_init_workload['remaining_adapter_slots_to_test']:
            input_ml_data_more_slots: Dict[str, Any] = self.__create_ml_input(
                rates=rates_to_test,
                sizes=sizes_to_test,
                served_adapters=served_adapters_to_test,
            ...
            input_ml_data_more_slots = None

        assert input_ml_data is not None or input_ml_data_more_slots is not None

        if input_ml_data is not None:
            ...
            total_throughput = 0
        if input_ml_data_more_slots is not None:
            output_ml_data_more_slots: Dict[str, Any] = predict_digital_twin(
                model_path=ml_model_path[model],
                x_features=input_ml_data_more_slots,
                y_features_to_predict=['total_throughput'],
            )
            total_throughput_more_slots: float = output_ml_data_more_slots['total_throughput'][0]
        else:
            total_throughput_more_slots = 0

        if total_throughput >= total_throughput_more_slots:
            ...
            updated_slots = server_init_workload['remaining_adapter_slots_to_test'][0]
            remaining_adapter_slots_to_test = server_init_workload['remaining_adapter_slots_to_test'][1:]

        ideal_throughput: float = sum(server_init_workload['rates'] + server_init_workload['pending_rates']) * (mean_input_length + mean_output_length)

        if max(total_throughput, total_throughput_more_slots) < (ideal_throughput * 0.9):
