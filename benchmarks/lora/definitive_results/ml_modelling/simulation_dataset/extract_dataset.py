...
def extract_results(path: str) -> List[Dict[str, Any]]:
    ...
    def extract_experiment_metric(path: str) -> Dict[str, Any]:
        ...
        # load arguments
        filenames: List[str] = glob.glob(os.path.join(path, 'arguments.json'))
        if len(filenames) != 1:
        ...
        # load metrics
        filenames: List[str] = glob.glob(os.path.join(path, 'simulation_results.json'))
        if len(filenames) != 1:
            raise ValueError(f'More than one output result file or none {filenames} for path {path}')
        with open(filenames[0]) as file:
        ...
        # load arrivals
        filenames: List[str] = glob.glob(os.path.join(path, 'arrivals.json'))
        if len(filenames) != 1:
            raise ValueError(f'More than one output result file or none {filenames} for path {path}')
        with open(filenames[0]) as file:
        ...
        output['rates'] = ' '.join([str(item) for item in arguments['served_adapters_rates']])

        # extract adapter sizes
        output['sizes'] = ' '.join([str(item) for item in arguments['served_adapters_sizes']])

        # extract total time
        total_time: float = float(arguments['total_time'])

        # compute ideal total throughput
        received_input_tokens: int = arrivals['total_arrivals_input_tokens']
        received_output_tokens: int = arrivals['total_arrivals_output_tokens']
...
def main():
    ...
    for model in ['llama-3.1-8b-instruct', 'qwen-2.5-7b-instruct']:
        ...
        path: str = os.path.join('', model, 'results')
