...
class SchedulerWithPreemption:
    ...
    def __init__(
        ...
        maximum_tokens: int = available_gpu_memory - adapter_slots * max_adapter_size
    ...
    def run_step(self, current_time: float) -> Deque[Request]:
        ...
        running_adapters: Set[str] = set()
        while self.running_requests:
            request: Request = self.running_requests.popleft()

            if request.remaining_tokens > 0:

                if request.blocks_in_use * BLOCK_SIZE < 1 + request.input_tokens + (request.output_tokens - request.remaining_tokens):
                    ...
                    if not preempted_itself:
                        ...
                        running_adapters.add(request.adapter_id)
        ...
        while self.waiting_requests:
            ...
            if request.adapter_id not in running_adapters and len(running_adapters) == self.adapter_slots:
                leftover_waiting_sequences.appendleft(request)
                continue
