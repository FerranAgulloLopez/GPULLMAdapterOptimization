...
PREFILL_CONSTANTS_PER_MODEL = {
    'llama-3.1-8b-instruct': {
        'constant_1': 0.29715263,
        'constant_2': 177.87454471
    },
    'qwen-2.5-7b-instruct': {
        'constant_1': 0.24776045,
        'constant_2': 150.93515525
    }
}

DECODE_CONSTANTS_PER_MODEL = {
    'llama-3.1-8b-instruct': {
        'constant_1': 3.15436184,
        'constant_2': 1.17466857,
        'constant_3': 106.4657897
    },
    'qwen-2.5-7b-instruct': {
        'constant_1': 3.03256212,
        'constant_2': 1.1319133,
        'constant_3': 106.14535535
    }
}


def latency_overhead_predictor_prefill(  # TODO should be improved
        x,
    ...
    loras_in_batch = x[0]  # not used
    rank = x[1]
    # assert np.all(total_input_tokens > 0)
    return constant_1 * rank + constant_2
...
def latency_overhead_predictor_decode(
    ...
    return constant_1 * loras_in_batch + rank ** constant_2 + constant_3
...
def plot_relationship(
        results_input_initial_points: List[Dict[str, float]],
        results_output_initial_points: List[Dict[str, float]],
        results_input_overheads: List[Dict[str, float]],
        results_output_overheads: List[Dict[str, float]],
        path: str,
        title: str,
) -> None:
    all_results = {
        'prefill': results_input_overheads,
        'decode': results_output_overheads
    }
    all_metrics_to_show = {
        'prefill': [
            'mean_ttft_ms',
            'mean_ttft_ms-overhead'
        ],
        'decode': [
            'mean_itl_ms',
    ...
    initial_points_input_reference: Dict[str, Dict[str, float]] = {}
    for result in results_input_initial_points:
        _id: str = f'{result["set_batch_size"]}_{result["set_input_length"]}_{result["set_output_length"]}'
        if 'mean_ttft_ms' not in result:
            raise ValueError('Missing mandatory results')
        initial_points_input_reference[_id] = {}
        for metric in all_metrics_to_show['prefill']:
            initial_points_input_reference[_id][metric.replace('-overhead', '')] = result[metric.replace('-overhead', '')]
    del results_input_initial_points
...
def plot_predictor(
    ...
    for label_results, metrics_to_show in all_metrics_to_show.items():
        ...
        predictor = latency_overhead_predictor_prefill if label_results == 'prefill' else latency_overhead_predictor_decode
...
def main():
    for model in ['llama-3.1-8b-instruct', 'qwen-2.5-7b-instruct']:
        ...
        duration, results_input_initial_points = extract_results(f'{model}/initial_points/input', 'initial_points')
        total_duration += duration

        duration, results_output_initial_points = extract_results(f'{model}/initial_points/output', 'initial_points')
        total_duration += duration

        duration, results_input_overheads = extract_results(f'{model}/overhead/input', 'overhead')
        total_duration += duration

        duration, results_output_overheads = extract_results(f'{model}/overhead/output', 'overhead')
        total_duration += duration

        plot_relationship(
            results_input_initial_points,
            results_output_initial_points,
            results_input_overheads,
