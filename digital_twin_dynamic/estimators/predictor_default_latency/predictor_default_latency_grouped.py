...
CONSTANTS_PER_MODEL = {
    ...
    'qwen-2.5-7b-instruct': {
        'constant_1': 0.03382976,
        'constant_2': 12.60266992
    }
}
...
def extract_results(path: str) -> List[Dict[str, float]]:
    ...
    def extract_experiment_metric(path: str) -> List[Dict[str, Any]]:
        ...
        # compute itl
        mean_itl_ms: float = float(metrics['mean_itl_ms'])

        # compute scheduler metrics (scheduler time and mean loras by batch)
        pattern = f'Timestamp: ((\d+\.\d+)(e-\d+)?). Scheduler time: ((\d+\.\d+)(e-\d+)?) seconds. Unique adapters by batch: ((\d+\.(\d+)?)?(e-\d+)?).Batched tokens: ((\d+\.?(\d+)?)?(e-\d+)?)'
        ...
        for data_point in progress_data:
            ...
            progress_batched_tokens.append(round(float(data_point[10])))
        ...
        output.append(
            {
                ...
                'mean_batched_tokens': np.mean(progress_batched_tokens),
                'aux': 0
            }
        )
        return output
...
def main():
    ...
    for model in ['qwen-2.5-7b-instruct']:
        results = extract_results(f'{model}/grouped')
